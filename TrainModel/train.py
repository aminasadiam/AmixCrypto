...
def Train5M():
    ...
    model = RandomForestRegressor(n_estimators=720, random_state=42)
    model.fit(X_train, y_train)

    last_data = data.iloc[-1:]  # آخرین داده‌ها برای پیش‌بینی
    future_predictions = []
    times = pd.to_datetime(data['Time'])
    future_dates = pd.date_range(start=times.max() + pd.Timedelta(minutes=5), periods=5, freq='5T')
    ...
    for i in range(5):
        ...
        new_row = {
            ...
            'Time': pd.to_datetime(last_data['Time'].values[i]) + pd.Timedelta(minutes=5)
        }

        last_data = pd.concat([last_data, pd.DataFrame([new_row])], ignore_index=True)

    actuals = [data[data['Time'] == future_dates[i] - pd.Timedelta(minutes=5)]['Close'].values[0] if not data[data['Time'] == future_dates[i] - pd.Timedelta(minutes=5)].empty else None for i in range(5)]
    result = {
...
def Train15M():
    ...
    future_predictions = []
    times = pd.to_datetime(data['Time'])
    future_dates = pd.date_range(start=times.max() + pd.Timedelta(minutes=15), periods=5, freq='15T')
    ...
    for i in range(5):
        ...
        new_row = {
            ...
            'SMA': last_data['SMA'].values[i],
            'RSI': last_data['RSI'].values[i],
            'Time': pd.to_datetime(last_data['Time'].values[i]) + pd.Timedelta(minutes=15)
        }

        last_data = pd.concat([last_data, pd.DataFrame([new_row])], ignore_index=True)

    actuals = [data[data['Time'] == future_dates[i] - pd.Timedelta(minutes=15)]['Close'].values[0] if not data[data['Time'] == future_dates[i] - pd.Timedelta(minutes=15)].empty else None for i in range(5)]
    result = {
        "accuracy": "N/A",  # دقت مدل برای پیش‌بینی‌های آینده را نمی‌توان به سادگی محاسبه کرد
        "predictions": future_predictions,
        "dates": future_dates.strftime('%m/%d/%Y, %H:%M:%S').tolist(),  # تبدیل به فرمت تاریخ مناسب
...
def Train30M():
    ...
    future_predictions = []
    times = pd.to_datetime(data['Time'])
    future_dates = pd.date_range(start=times.max() + pd.Timedelta(minutes=30), periods=5, freq='30T')
    ...
    for i in range(5):
        ...
        new_row = {
            ...
            'SMA': last_data['SMA'].values[i],
            'RSI': last_data['RSI'].values[i],
            'Time': pd.to_datetime(last_data['Time'].values[i]) + pd.Timedelta(minutes=30)
        }

        last_data = pd.concat([last_data, pd.DataFrame([new_row])], ignore_index=True)

    actuals = [data[data['Time'] == future_dates[i] - pd.Timedelta(minutes=30)]['Close'].values[0] if not data[data['Time'] == future_dates[i] - pd.Timedelta(minutes=30)].empty else None for i in range(5)]
    result = {
        "accuracy": "N/A",  # دقت مدل برای پیش‌بینی‌های آینده را نمی‌توان به سادگی محاسبه کرد
        "predictions": future_predictions,
        "dates": future_dates.strftime('%m/%d/%Y, %H:%M:%S').tolist(),  # تبدیل به فرمت تاریخ مناسب
